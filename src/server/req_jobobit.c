...
static void complete_job_termination(job *pjob);
	...
	switch (pjob->ji_qs.ji_substate) {
		...
		case JOB_SUBSTATE_STAGEDEL:
			...
			if (ptask->wt_type != WORK_Deferred_Reply) { /* first time in */
				...
				} else { /* preq == 0, no files to delete so terminate */
					ack_obit(handle, pjob->ji_qs.ji_jobid);
					complete_job_termination(pjob);
					return;
				}
			}
 ...
 * @brief
 * complete_job_termination
 * 	Terminate job after freeing resources and adding accounting logs
 *
 * @param[in]   pjob 	- job to terminate
 *
 * @return void
 *
 */
static void
complete_job_termination(job *pjob)
{
	char  *rec;
...
RetryJob:
	...
	if ((exitstatus == JOB_EXEC_FAILHOOK_DELETE) ||
		...
		/* normal mom end-of-job processing */
