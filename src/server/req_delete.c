	...
	if (jid) {
		if (pbs_idx_find(idx, (void **) &jid, (void **) &data, NULL) == PBS_IDX_RET_OK) {
				...
				log_eventf(PBSEVENT_DEBUG, PBS_EVENTCLASS_SERVER, LOG_DEBUG,
					  __func__, "job %s has found and deleted from the deljob idx", jid);
		} else 
			log_eventf(PBSEVENT_DEBUG, PBS_EVENTCLASS_SERVER, LOG_DEBUG,
...
init_deljoblist_rply(struct batch_request *preq)
{
	...
	for (i = 0; jobids[i]; i++) {
		if (pbs_idx_find(preply->brp_un.brp_deletejoblist.undeleted_job_idx, (void **) &jobids[i], (void **) &data, NULL) == PBS_IDX_RET_OK)
			log_errf(-1, __func__, "Found jobid %s in the tree!", jobids[i]);
	}
}
