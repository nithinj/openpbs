 ...
 * @param[in]	name_out - output name
 *
 * @return void
 */
void
searchable_nodename(char *name_in, char *nodename)
{
	char *src = name_in;
	char *dest = nodename;
	...
	while ((src - name_in < PBS_MAXDEST) && (*src != '\0') && (*src != '/'))
		*dest++ = *src++;
	*dest = '\0';
}
 ...
 * 		refresh from database if modified.
...
find_nodebyname(char *nodename_in)
{
	...
	searchable_nodename(nodename_in, nodename);
	return node_recov_db(nodename, find_node_fromcache(nodename));
}
